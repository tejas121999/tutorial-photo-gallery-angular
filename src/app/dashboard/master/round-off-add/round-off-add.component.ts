...
@Component({
  selector: "app-round-off-add",
  templateUrl: "./round-off-add.component.html",
  styleUrls: ["./round-off-add.component.scss"],
})
export class RoundOffAddComponent implements OnInit {
  roundOffAddForm: FormGroup;
  branch_token: any;
  login_token: any;
  ...
  async initializeData() {
    this.roundOffAddForm = this.fb.group({
  ...
  createPayload() {
    ...
    this.payload[0].round_of_data[0] = {
      round_of_name: this.roundOffAddForm.get("name")?.value,
      alias_name: this.roundOffAddForm.get("alias")?.value,
      maintain_bill_by_bill: this.roundOffAddForm.get("billwiseBalance")?.value,
      inventory_value_affect:
        this.roundOffAddForm.get("affectOnInventory")?.value,
      type_of_ledger: this.roundOffAddForm.get("typeOfLedger")?.value,
      rounding_method:
        this.roundOffAddForm.get("roundingMethod")?.value || "rounding_method",
      rounding_limit: this.roundOffAddForm.get("roundingLimit")?.value || "0",
      gst_applicability: this.roundOffAddForm.get("GSTApplicable")?.value,
      openning_balance:
        this.roundOffAddForm.get("openingBalance")?.value || "0",
      cost_center_applicable: this.roundOffAddForm.get("costCenterApplicable")
        ?.value,
    };
    if (!this.payload[0].round_of_data[0].gst_applicable_data) {
      this.payload[0].round_of_data[0].gst_applicable_data = {};
    }
    this.payload[0].round_of_data[0].gst_applicable_data.hsn_sac_detail_data = {
      hsn_sac_detail: this.roundOffAddForm.get("HSNDetails")?.value,
      classification_create_data: {
        GSTRateRelatedDetail_data: {
          gstRetDetail: this.roundOffAddForm.get("GSTRateDetails")?.value,
          type_of_suppile: this.roundOffAddForm.get("typeOfSupply")?.value,
        },
      },
    };
  }
  ...
  createRoundOffAdd() {
    this.isLoading = true;
    if (
      this.roundOffAddForm.get("name")?.value &&
      this.roundOffAddForm.get("alias")?.value &&
      this.roundOffAddForm.get("billwiseBalance")?.value &&
      this.roundOffAddForm.get("affectOnInventory")?.value &&
      this.roundOffAddForm.get("typeOfLedger")?.value
    ) {
      this.resetPayloadValuesToNull(this.payload);
      this.createPayload();
      console.log("Payload:", this.payload);
      this.apiService.addRoundOffAdd(this.payload).subscribe({
        next: (response: any) => {
          this.isLoading = false;
          if (response?._Object?.length !== 0) {
            this.router.navigate(["/dashboard/master/round-off-add-list"], {
      ...
      this.isLoading = false;
      this.roundOffAddForm.markAllAsTouched();
    }
  }
}
