...
@Component({
  selector: "app-round-off",
  templateUrl: "./round-off.component.html",
  styleUrls: ["./round-off.component.scss"],
})
export class RoundOffComponent implements OnInit {
  roundOffLessForm: FormGroup;
  branch_token: any;
  login_token: any;
  ...
  createPayload() {
    ...
    this.payload[0].round_of_data[0] = {
      round_of_name: this.roundOffLessForm.get("name")?.value,
      alias_name: this.roundOffLessForm.get("alias")?.value,
      ...
      type_of_ledger: this.roundOffLessForm.get("typeOfLedger")?.value,
      rounding_method:
        this.roundOffLessForm.get("roundingMethod")?.value || "rounding_method",
      rounding_limit: this.roundOffLessForm.get("roundingLimit")?.value || "0",
      gst_applicability: this.roundOffLessForm.get("GSTApplicable")?.value,
      openning_balance:
        this.roundOffLessForm.get("openingBalance")?.value || "0",
      cost_center_applicable: this.roundOffLessForm.get("costCenterApplicable")
        ?.value,
    };
    ...
    this.payload[0].round_of_data[0].gst_applicable_data.hsn_sac_detail_data = {
      hsn_sac_detail: this.roundOffLessForm.get("HSNDetails")?.value,
      classification_create_data: {
        GSTRateRelatedDetail_data: {
          gstRetDetail: this.roundOffLessForm.get("GSTRateDetails")?.value,
          type_of_suppile: this.roundOffLessForm.get("typeOfSupply")?.value,
        },
      },
    };
  }
  ...
  createRoundOffLess() {
    ...
    if (this.roundOffLessForm.get("name")?.value) {
      ...
      this.apiService.addRoundOffLess(this.payload).subscribe({
        next: (response: any) => {
      ...
      this.isLoading = false;
      this.roundOffLessForm.markAllAsTouched();
    }
  }
}
