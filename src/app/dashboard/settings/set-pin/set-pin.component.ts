...
export class SetPinComponent implements OnInit {
  ...
  deleteNumber() {
    const lastFilledIndex = this.pin.map(digit => digit !== '').lastIndexOf(true);
  ...
  async submitPin() {
    if (this.pin.every(digit => digit !== '')) {
      ...
      try {
        ...
        await this.appPreference.presentToast('PIN set successfully', 2000, 'bottom', 'success');
        this.router.navigate(['/dashboard/settings']);
      } catch (error) {
        console.error('Error saving PIN:', error);
        await this.appPreference.presentToast('Failed to set PIN', 2000, 'bottom', 'danger');
      }
    }
  }
}
