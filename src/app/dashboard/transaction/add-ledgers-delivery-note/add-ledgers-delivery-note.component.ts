...
@Component({
  selector: "app-add-ledgers-delivery-note",
  templateUrl: "./add-ledgers-delivery-note.component.html",
  styleUrls: ["./add-ledgers-delivery-note.component.scss"],
})
export class AddLedgersDeliveryNoteComponent implements OnInit {
  pageSize = 15;
  currentPage = 1;
  showSearchbar = false;
  branch_token: any;
  ...
  async getLedgerList() {
    ...
    this.apiService.getLedgerList(body).subscribe(
      (response: any) => {
        ...
        //   console.log("Ledger Detail:", element);
        //   var temp = JSON.parse(element.ledger_detail);
        //   console.log("Parsed Ledger Detail:", temp);
        //   // var data = {
