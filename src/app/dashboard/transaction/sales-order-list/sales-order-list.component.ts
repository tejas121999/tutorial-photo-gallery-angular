...
export class SalesOrderListComponent implements OnInit {
  ...
  getSalesOrderList() {
    ...
    this.apiService.getSalesOrderVoucherList(temp).subscribe(
      (response: any) => {
        if (response?._Object) {
          console.log("Sales Order List:", response._Object);
          this.data = response?._Object;
          this.results = [...this.data];
          // this.filterByDate();
          // You can store the sales order list in a variable if needed
        } else {
